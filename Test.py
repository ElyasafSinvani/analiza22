...
class D:
    ...
    @staticmethod
    def brackfunc(func,
                  brack=0):  # returns a string with complete brackets like (sin(x)) or (sin(x)cos(x)) from given string
    ...
    @staticmethod
    def fvalue(funct,
               val=''):  # returns a function if value not specified else returns the value of a function at a particular point
        def fval(val):
            func = funct
            try:
    ...
    def simplify(func):  # simplifies a term like combining powers, calculating the const of the term, etc.
        i = re.search(
            r'(?<!([a-wA-Zyz\d/\^][(]))(?P<f1>[a-wA-Zyz]+)(?P<pow1>\^[\d]+)?\(x\).*?(?<!([a-wA-Zyz\d/\^][(]))(?P=f1)(?P<pow2>\^[\d]+)?(?P<x1>\(x\))',
            func)  # to find 'f(x)...f(x)'
        func_1 = ''
        while i:
    ...
    def chainrule(self, func):  # calculates d /dx (f(g(...(x))))
        cr = ''
        fsep = re.split(r'(\)|\()',
                        func)  # for eg, 'cos(x)sin(x)' becomes ['cos', '(', 'x', ')', 'sin', '(', 'x', ')', '']
    ...
    def productrule(self, func):  # calculates d /dx (uvw...)
        ...
        pr = D.listjoin(prdlist, 0, len(prdlist),
                        sgn)  # string which contains the total product rule (sum of derivatives in prdlist)
        if sgn == '-':
            pr = '-' + pr
        return pr
...
def GUI():  # GUI is written inside function to have a choice of using/not using GUI
    ...
    Label(w, text='Enter  f(x)  = ', font=('Times New Roman', 14), padx=10, pady=30, fg='blue').grid(column=0,
                                                                                                     row=0)  # Enter f(x) =

    fx = Entry(w, font=('Times New Roman', 14), justify='center', bd=3, width=40)  # f(x) numerator input
    fx.grid(row=0, column=1)

    showdenom = Button(w, text='f(x)/g(x)', font=('Times New Roman', 14), bg='#D4FB90', width=7, bd=3,
                       command=expand)  # f(x)/g(x) Button
    ...
    diff = Button(w, text=' Differentiate! ', font=('Times New Roman', 14), bg='#D4FB90', width=10, bd=3,
                  command=diff_fx)  # Differentiate Button
    diff.grid(row=5, column=1)

    fx_plot = Button(w, text="Plot f (x)", font=('Times New Roman', 14), width=7, bg='#D4FB90', bd=3,
                     command=plot_fx)  # Plot f(x) Button
    fx_plot.grid(row=2, column=1)

    f1x_plot = Button(w, text="Plot f '(x)", font=('Times New Roman', 14), width=7, bg='#D4FB90', bd=3,
                      command=plot_f1x)  # Plot f'(x) Button
    ...
    calc_n_der = Button(w, text='Calculate nᵗʰ Derivative!', font=('Times New Roman', 14), bg='#D4FB90', bd=3,
                        command=nth_der)  # Calculate nᵗʰ derivative Button
    calc_n_der.grid(row=9, column=1)

    Label(w, text='Enter x coordinate :  ', font=('Times New Roman', 14), padx=10, fg='blue').grid(column=2,
                                                                                                   row=0)  # Enter x coordinate:

    x_coor = Entry(w, font=('Times New Roman', 14), bd=3, width=15)  # x coordinate input
    x_coor.grid(row=0, column=3)

    Label(w, text='Tangent Equation of f(x) is :  ', font=('Times New Roman', 14), padx=10, fg='blue').grid(column=2,
                                                                                                            row=1)  # Tangent Equation f(x) is:

    tan = Entry(w, font=('Times New Roman', 14), bd=3, width=15)  # Display tangent equation
    tan.grid(row=1, column=3)

    Label(w, text='Normal Equation of f(x) is :  ', font=('Times New Roman', 14), padx=10, fg='blue').grid(column=2,
                                                                                                           row=2)  # Normal Equation f(x) is:

    nor = Entry(w, font=('Times New Roman', 14), bd=3, width=15)  # Display normal equation
    nor.grid(row=2, column=3)

    tan_get = Button(w, text="Get Tangent!", font=('Times New Roman', 14), bd=3, bg='#D4FB90',
                     command=get_tan)  # Get tangent Button
    tan_get.grid(row=3, column=2)

    nor_get = Button(w, text="Get Normal!", font=('Times New Roman', 14), bd=3, bg='#D4FB90',
                     command=get_nor)  # Get normal Button
    nor_get.grid(row=3, column=3)

    tan_plot = Button(w, text="Plot tangent of f(x)!", font=('Times New Roman', 14), bd=3, bg='#D4FB90',
                      command=plot_tan)  # Plot tangent of f(x) Button
    tan_plot.grid(row=5, column=2)

    nor_plot = Button(w, text="Plot normal of f(x)!", font=('Times New Roman', 14), bd=3, bg='#D4FB90',
                      command=plot_nor)  # Plot normal of f(x) Button
    nor_plot.grid(row=5, column=3)

    quitbutton = Button(w, text="Quit!", font=('Bold Times New Roman', 14), bg='#FF4B4B', fg='white', width=10, bd=3,
                        command=quitb)  # Plot normal of f(x) Button
