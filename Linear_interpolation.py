...
table3 = ((6.8, 4.35), (7.2, 2.74), (8.3, 5.70))
table4 = {(0, 0): [1, 0], (1, 1): [1.2, 0.112463],
          (2, 2): [1.3, 0.167996], (3, 3): [1.4, 0.222709]}
X_to_find = 2.5

# PAR5
values_table = tuple([[5.5,6.8, 7.2, 8.3], [3.14,4.35, 2.74, 5.70]])
x_f = 7.0
...
def Interrogation_of_Lagrange(X):
    x = 7  # you can change your param to find here
    ...
    print(f'(x - {X[1][0]}) / ({X[0][0]} - {X[1][0]})) * ((x - {X[2][0]}) / ({X[0][0]} - {X[2][0]}) == > {L0}')
    print(f'(x - {X[0][0]}) / ({X[1][0]} - {X[0][0]})) * ((x - {X[2][0]}) / ({X[1][0]} - {X[2][0]}) == > {L1}')
    print(f'(x - {X[0][0]}) / ({X[2][0]} - {X[0][0]})) * ((x - {X[1][0]}) / ({X[2][0]} - {X[1][0]}) == > {L2}')
...
def Spline_Kobe(arr, x_f1):
    ...
    for i in range(1, len(arr[0]) - 1):
        di.append((6 / (hi[i - 1] + hi[i])) * (
                ((arr[1][i + 1] - arr[1][i]) / hi[i]) - ((arr[1][i] - arr[1][i - 1]) / hi[i - 1])))
    for i in range(size):
        helper = []
        for j in range(size):
            if i == j:
    ...
    for i in range(len(arr[0])):
        ...
        if arr[0][i] > x_f1:
            x_1 = arr[0][i - 1]
            x_2 = arr[0][i]
            s_x = (((x_2 - x_f1) ** 3) * final_result1[counter - 1] + ((x_f1 - x_1) ** 3) * final_result1[counter]) / (
                    6 * hi[counter]) + (((x_2 - x_f1) * arr[1][i - 1] + (x_f1 - x_1) * arr[1][i]) / hi[counter]) - (
                          ((x_2 - x_f1) * final_result1[counter - 1] + (x_f1 - x_1) * final_result1[counter]) * hi[
                      counter]) / 6
            break
    return s_x
